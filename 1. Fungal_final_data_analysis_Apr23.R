...
#setwd("c:/Users/drcjb/Dropbox/Danish_Plantago_microbiomes/Fungi")
#setwd("/Users/jsv736/Dropbox/Danish_Plantago_microbiomes/Fungi/")


data <- read.table(file = "Final_analysis_files/data.txt", sep = "\t", header = TRUE)
#data <- read.table(file = "dada2_seq_table.txt", sep = "\t", header = TRUE)
Taxonomy <- data.frame("Taxa" = data[,173])
data <- data[,-c(173)]
...
data <- t(data)
min(rowSums(data))
which(rowSums(data) == 520)
dim(data)
#Taxonomy <- as.data.frame(str_split_fixed(Taxonomy$Taxa, ";", 7))
#colnames(Taxonomy) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
#rownames(Taxonomy) <- colnames(data)

x <- readRDS("Tobias_files/dada2_table_extracted_filtered_and_clustered_assigned.RDS")
class(x)
x1 <- x$full_table
Taxonomy <- x$dada2_taxonomy
colnames(Taxonomy) <- c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus", "Species")
rownames(Taxonomy) <- colnames(data)


#Note that the blanks had too few reads and were thefore removed (sub 500 reads). 
#Additionally, non fungal reads are excised as part of ITSx, therefore all OTU were assigned to fungi without filtering needed here. 

data <- as.matrix(data)
library(pracma)
#rel_data <- data
rel_data <- nthroot(data, 4)
rel_data <- (make_relative(rel_data)*100)
rel_data <- as.data.frame(rel_data)

rel_data <- rel_data[-c(109),]
x <- (colSums(rel_data != 0)) > 0 #OTU as columns
rel_data <- rel_data[, x == "TRUE"] #Removes taxa that are all 0s
Taxonomy <- Taxonomy[ x == "TRUE",] #Removes taxa that are all 0s
dim(Taxonomy)


metadata <- read.table(file = "Final_analysis_files/metadata.txt", sep = "\t", header = TRUE)
metadata <- metadata[-c(109),] #Sample 109 seems to me missassigned. 

#Richness <- rowSums(rel_data > 0)
Richness <- rarefy(data, 520, se = FALSE, MARGIN = 1)
Richness <- Richness[-(109)]
Original_richness <- Richness
...
spat_data <- as.matrix(cbind(metadata[,16], metadata[,15]))
...
norm_metadata_prep <- (cbind("pH" = metadata$pH, "EC" = metadata$EC, "BulkDensity" = metadata$BulkDensity_g.cm3,
...
metadata <- data.frame("Stratum" = metadata$Stratum, "Habitat" = metadata$Habitat, "Site" = metadata$Site, "PlantID" = metadata$PlantID, 
                       "Light" = metadata$Light, "Geography3" = metadata$Geography3,
                       "Island" = metadata$Island, "PCNM1" = metadata$PCNM1, 
                       "PCNM2" = metadata$PCNM2, "Original_richness" = Original_richness,
                       norm_metadata)

metadata$Habitat[which(metadata$Habitat == "SeasideMeadow")] <- "Meadow"
metadata <- droplevels(metadata)

mean(rowSums(rel_data))

a <- princomp(cbind(metadata$pH, metadata$EC, metadata$BulkDensity))
summary(a)
soilPC1 <- a$scores[,1]
soilPC2 <- a$scores[,2]
a <- princomp(cbind(metadata$ColdestQuartMeanTemp, metadata$AnnPrec, metadata$MeanAnnTemp))
climPC1 <- a$scores[,1]
climPC2 <- a$scores[,2]
a$loadings
summary(a)
cor.test(PCNM1, PCNM2)

Moran.I(soilPC1, dist_matrix)

library(Hmisc)
res2 <- rcorr(as.matrix(cbind(norm_metadata, PCNM1, PCNM2)))
corrplot(res2$r, #type="upper", 
         p.mat = res2$P, sig.level = 0.01, insig = "blank")

library(pracma)
untargeted_chem <- read.table(file = "Final_analysis_files/Untargeted_chem.txt", sep = "\t", header = TRUE)
untargeted_chem <- untargeted_chem[-c(109),]
...
targeted_chem <- read.table(file = "Final_analysis_files/Targeted_chem.txt", sep = "\t", header = TRUE)
targeted_chem <- targeted_chem[-c(109),]
...
#all_bray_data <- dist(rel_data, method = "ait") # Creates a similarity matrix (Jaccard is the type), based on presence/absence
...
#saveRDS(Nmds, file = "/Users/christopherbarnes/Dropbox/Danish_Plantago_microbiomes/Combined_analyses/fungal_nMDS.RDS")
#saveRDS(metadata, file = "/Users/christopherbarnes/Dropbox/Danish_Plantago_microbiomes/Combined_analyses/fungal_metadata.RDS")
...
header <- paste(metadata$PlantID, metadata$Stratum, sep = "_")
header <- str_replace(header, " ", "_")

which(colnames(rel_data) == "SeqVar2333")
rel_data[, 2332]
t_rel_data <- as.data.frame(t(rel_data))
colnames(t_rel_data) <- header
write.table(t_rel_data, file = "t_final_rel_data.txt")
...
#saveRDS(x, file = "Fungal_Venn_data.RDS")
#saveRDS(x, file = "Fungal_Venn_data_simplified.RDS")

plot(metadata$MeanAnnTemp, metadata$EC)
cor.test(metadata$pH, metadata$BulkDensity)
cor.test(metadata$BulkDensity, metadata$EC)
...
adonis2(root_data ~ root_metadata$Habitat + root_metadata$Island +
          root_metadata$pH + root_metadata$AnnPrec, group = root_metadata$Site)
...
cor.test(soil_metadata$Richness, soil_metadata$BulkDensity_g.cm3, method = "spearman")
...
adonis2(dam_data ~ dam_metadata$Habitat + dam_metadata$Island +
  dam_metadata$ColdestQuartMeanTemp, 
        group = dam_metadata$Site)
...
adonis2(dam_bray_data ~ dam_targeted_chem$VerbascosideHeight)
adonis2(dam_bray_data ~ dam_targeted_chem$AucubinHeight)



dam_MDS_data <- monoMDS(dam_bray_data) # Performs multidimensional scaling for the data
plot(dam_MDS_data)

Nmds1 <- dam_MDS_data$points[,1] #Extracts dimension 1 as a vector
Nmds2 <- dam_MDS_data$points[,2] #Extracts dimension 2 as a vector

Nmds <-cbind(Nmds1, Nmds2) #Makes a new sheet with dimension1 and 2
Nmds <- as.data.frame(Nmds) #Creates a new dataframe with dimensions 1 and 2

ggplot(data = Nmds, aes(y = Nmds2, x = Nmds1, Type="p", 
  colour = dam_metadata$Habitat)) + 
  geom_point(size = 4, alpha = 0.5) + labs(x="Dimension 1", y = "Dimension 2", colour = NULL, shape = NULL) + 
  theme(plot.title=element_text(hjust=0)) + 
  ggtitle("D") +
  theme(legend.position = "right") +
...
a <- prcomp(dam_untargeted_chem)
summary(a)
plot(a)
dam_untar_chem_PC1 <- a$x[,1]
dam_untar_chem_PC2 <- a$x[,2]
dam_untar_chem_PC3 <- a$x[,3]
dam_untar_chem_PC4 <- a$x[,4]

adonis2(dam_bray_data ~ dam_untar_chem_PC1)
adonis2(dam_bray_data ~ dam_untar_chem_PC2)
adonis2(dam_bray_data ~ dam_untar_chem_PC3)
adonis2(dam_bray_data ~ dam_untar_chem_PC4)



dam_targeted_chem$X10.acetoxymajorHeight
dam_untar_chem_PC4
...
adonis2(undam_data ~ undam_metadata$AnnPrec)
...
adonis2(undam_data ~ undam_metadata$Habitat + undam_metadata$Island + undam_untar_chem_PC1, 
...
adonis2(undam_bray_data ~ undam_targeted_chem$GeniposideHeight)
adonis2(undam_bray_data ~ undam_targeted_chem$GardosideHeight)
adonis2(undam_bray_data ~ undam_targeted_chem$MelittosideHeight)
adonis2(undam_bray_data ~ undam_targeted_chem$X10.acetoxymajorHeight)
adonis2(undam_bray_data ~ undam_targeted_chem$VerbascosideHeight)
adonis2(undam_bray_data ~ undam_targeted_chem$AucubinHeight)


b <- prcomp(undam_untargeted_chem)
summary(b)
plot(b)
undam_untar_chem_PC1 <- b$x[,1]
undam_untar_chem_PC2 <- b$x[,2]
undam_untar_chem_PC3 <- b$x[,3]
undam_untar_chem_PC4 <- b$x[,4]

adonis2(undam_bray_data ~ undam_untar_chem_PC1)
adonis2(undam_bray_data ~ undam_untar_chem_PC2)
adonis2(undam_bray_data ~ undam_untar_chem_PC3)
adonis2(undam_bray_data ~ undam_untar_chem_PC4)



x <- which(rownames(undam_data) == "S043")
undam_bray_data <- vegdist(undam_data[-x,], Type = "bray", binary = FALSE) # Creates a similarity matrix (Jaccard is the type), based on presence/absence

undam_MDS_data <- monoMDS(undam_bray_data) # Performs multidimensional scaling for the data
plot(undam_MDS_data)

Nmds1 <- undam_MDS_data$points[,1] #Extracts dimension 1 as a vector
Nmds2 <- undam_MDS_data$points[,2] #Extracts dimension 2 as a vector

Nmds <-cbind(Nmds1, Nmds2) #Makes a new sheet with dimension1 and 2
Nmds <- as.data.frame(Nmds) #Creates a new dataframe with dimensions 1 and 2

ggplot(data = Nmds, aes(y = Nmds2, x = Nmds1, Type="p", 
  colour = undam_metadata$Habitat[-x])) + 
  geom_point(size = 4, alpha = 0.5) + labs(x="Dimension 1", y = "Dimension 2", colour = NULL, shape = NULL) + 
  theme(plot.title=element_text(hjust=0)) + 
  ggtitle("D") +
  theme(legend.position = "right") +
...
#saveRDS(Big_data, "c:/Users/drcjb/Dropbox/Danish_Plantago_microbiomes/Combined_analyses/Fungal_big_data.RDS")
